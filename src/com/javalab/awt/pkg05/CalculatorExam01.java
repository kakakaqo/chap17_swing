...
/**
 * Swing 사칙연산 계산기 프로그램 만들기
 */

public class CalculatorExam01 extends JFrame implements ActionListener {

	private JTextField tf1, tf2, tf3;
	private JButton addBtn, subtracBtn, multipleBtn, divideBtn;

	public CalculatorExam01() {
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		tf1 = new JTextField(10); // 텍스트 필드 입력칸 객체 생성
		tf2 = new JTextField(10); // 텍스트 필드 입력칸 객체 생성
		tf3 = new JTextField(10); // 텍스트 필드 입력칸 객체 생성
		tf3.setEditable(false); // 편집불가(결과만 보여줄꺼니까)

		addBtn = new JButton("+"); // 더하기 버튼
		subtracBtn = new JButton("-"); // 빼기 버튼
		multipleBtn = new JButton("*"); // 곱하기 버튼
		divideBtn = new JButton("/"); // 나누기 버튼

		addBtn.addActionListener(this); // 버튼에 액션 리스너 부착
		subtracBtn.addActionListener(this); // 버튼에 액션 리스너 부착
		multipleBtn.addActionListener(this); // 버튼에 액션 리스너 부착
		divideBtn.addActionListener(this); // 버튼에 액션 리스너 부착

		// 4행 3열의 그리드 레이아웃이 적용된 패널 생성
		// 칸을 나누고 순차적으로 하나의 컴포넌트를 추가해 나간다.
		// 남는 칸은 빈칸으로 남아 있음.
		...
		// 컨텐트 패널은 JFrame의 기본 컨테이너이며 프렝미 내의 콘텐츠 및 구성 요소 관리를 담당
